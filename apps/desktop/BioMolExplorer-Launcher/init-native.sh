...
# freshly installed .deb is always detected as new (even at the same version).
_bundled_id() {
  [ -f "$SRC_TAR" ] || return 1
  if command -v sha256sum >/dev/null 2>&1; then
    sha256sum "$SRC_TAR" 2>/dev/null | cut -c1-16
  elif command -v shasum >/dev/null 2>&1; then
    shasum -a 256 "$SRC_TAR" 2>/dev/null | cut -c1-16
  else
    wc -c < "$SRC_TAR" 2>/dev/null | tr -d ' '   # fallback: size in bytes
  fi
}

# Reads the app version bundled inside the source tarball (for display only).
_bundled_version() {
  [ -f "$SRC_TAR" ] || return 1
  local top
  top=$(tar -tzf "$SRC_TAR" 2>/dev/null | head -1 | cut -d/ -f1)
  [ -n "$top" ] || return 1
  tar -xzOf "$SRC_TAR" "$top/package.json" 2>/dev/null \
    | grep -m1 '"version"' \
    | sed -E 's/.*"version"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/'
}

extract_source() {
  local installed_id="" bundled_id="" ver=""
  [ -f "$APP_DIR/.buildid" ] && installed_id=$(cat "$APP_DIR/.buildid" 2>/dev/null)
  bundled_id=$(_bundled_id) || bundled_id=""
  ver=$(_bundled_version) || ver=""

  # No bundled source in the package: keep any existing install, else fail.
  if [ ! -f "$SRC_TAR" ]; then
    [ -f "$APP_DIR/package.json" ] && { ok "Installation found at $APP_DIR"; return; }
    fail "Package not found: $SRC_TAR" "Try reinstalling BioMolExplorer via .deb."
  fi

  # First run: full extraction.
  if [ ! -f "$APP_DIR/package.json" ]; then
    step "Installing BioMolExplorer${ver:+ v$ver} at $INSTALL_DIR..."
    mkdir -p "$APP_DIR"
  ...
  if [ -n "$bundled_id" ] && [ "$installed_id" = "$bundled_id" ]; then
    ok "Installation up to date at $APP_DIR${ver:+ (v$ver)}"
    return
  fi

  # Different build (or unknown): refresh the code over the existing install.
  # datasets/, resultados/, apps/api/uploads/ and node_modules are NOT in the
  # tarball, so this overlay never touches user data or installed dependencies.
  step "Updating BioMolExplorer code${ver:+ (v$ver)}..."
  find "$APP_DIR" -type d -name '__pycache__' -prune -exec rm -rf {} + 2>/dev/null || true
  rm -rf "$APP_DIR/apps/web/.next" 2>/dev/null || true
  tar -xzf "$SRC_TAR" -C "$APP_DIR" --strip-components=1 \
    || fail "Failed to update source code." "The file may be corrupted. Reinstall the .deb."
  printf '%s\n' "$bundled_id" > "$APP_DIR/.buildid"
  printf '%s\n' "$ver"        > "$APP_DIR/.version"
  NEEDS_NPM_INSTALL=1   # a code update may change JS deps -> force a sync
  ok "Updated${ver:+ to v$ver}!"
}
