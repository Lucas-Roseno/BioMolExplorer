...
def perform_redocking(base_input_path:str, target:str, base_output_path:str, pdb_codes:Optional[List[Tuple[str, str, str, str]]]=None, 
    ...
    try:
        ...
        if pdb_codes is None:
            print(f"DEBUG: Loading pdb_codes from {base_input_path}")
            pdb_codes = f1.csv_to_dataframe('pdb_codes')
            print(f"DEBUG: Columns found: {pdb_codes.columns.tolist()}")
        ...
        if os.path.exists(base_output_path):
            [os.remove(base_output_path + file) for file in os.listdir(base_output_path) if file.endswith('.vina')]
